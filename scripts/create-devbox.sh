...
get_install_cmd() {
    ...
    case "$image" in
        *fedora*|*centos*|*rhel*|*alma*|*rocky*)
            echo "dnf install -y git zsh curl wget nodejs pnpm python3 uv docker-cli docker-compose gcc gcc-c++ make openssl openssl-devel"
            ;;
        *ubuntu*|*debian*)
            echo "apt-get update && apt-get install -y git zsh curl wget nodejs npm python3 python3-pip docker.io docker-compose build-essential libssl-dev ca-certificates"
            ;;
        *archlinux*|*arch:*)
            echo "pacman -Syu --noconfirm git zsh curl wget nodejs npm python uv docker-cli docker-compose base-devel openssl"
            ;;
        *)
...
TOOLS="node npm npx pnpm python3 uv uvx"
if [ "$HAS_DOCKER" = true ]; then
